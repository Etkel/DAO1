...
//DZ 1------------------------------------------------------------------------
            // TODO: get ID
            // SELECT - X
            try (PreparedStatement ps = conn.prepareStatement("SELECT " + id.getName() + " FROM " + table)) {
...
//DZ 2--------------------------------------------------------
    public List<T> getAll(Class<T> cls, String... arg) {
            ...
            try (Statement st = conn.createStatement()) {
                try (ResultSet rs = st.executeQuery("SELECT " + sb + " FROM " + table)) {
                    ...
                    while (rs.next()) {
                        ...
                        for (String tmp : arg) {
                            Field field = cls.getDeclaredField(tmp);
                            field.setAccessible(true);
                            field.set(t, rs.getObject(tmp));
                        }
                        res.add(t);
                    }
                }
